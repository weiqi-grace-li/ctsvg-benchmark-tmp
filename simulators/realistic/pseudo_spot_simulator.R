...
pseudo_data_ori = setRefClass(
  ...
  fields = list(
    ...
    gene_cell_drop        = "dgCMatrix",      # from gene_dropout_frac.csv (genes x cells, as read)
    pseudo_meta           = "data.frame"      # from pseudo_meta.csv
  ),
  methods = list(
    initialize = function(cell_count, cell_meta, pseudo_meta, cell_spot_prop, gene_cell_drop) {
      # original checks and fields
      if (any(rownames(cell_count) != rownames(gene_cell_drop))) {
        stop("Number or order of genes in cell_count and gene_cell_drop doesn't match!")
      }
      if (any(colnames(cell_count) != rownames(cell_meta)) ||
          any(colnames(cell_count) != rownames(cell_spot_prop)) ||
          any(colnames(cell_count) != colnames(gene_cell_drop))
          ){
      ...
      .self$gene_cell_drop = gene_cell_drop
...
pseudo_simulator <- setRefClass("pseudo_simulator",
                                   ...
                                   methods = list(
                                     initialize = function(data_ori, newcount_list, para_list, seed, special_genes) {
                                       .self$seed = seed
                                       ...
                                       .self$merge(data_ori, newcount_list, para_list)
                                       .self$form_spot(data_ori)
                                     },

                                     #' @description Run full simulation.
                                     merge = function(data_ori, newcount_list, para_list){
