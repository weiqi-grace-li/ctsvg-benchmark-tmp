...
scalable_sim <- setRefClass("scalable_sim",
                                    fields = list(
                                      dispersion = "numeric",
                                      seed = "numeric",
                                    ...
                                    methods = list(
                                      initialize = function(dispersion = 0.7, seed = 42, n_cell_types = 3, n_side = 10, n_genes = 100) {
                                        .self$dispersion <- dispersion
                                        .self$seed <- seed
                                        .self$cell_type_proportion <- rep(1/n_cell_types, n_cell_types)
                                        names(.self$cell_type_proportion) = paste0("Type", 1:n_cell_types)
                                        .self$simulate(n_cell_types, n_side, n_genes)
                                      },
                                      
                                      simulate = function(n_cell_types, n_side, n_genes) {
                                        set.seed(.self$seed)
                                        n_cells = n_side^2*20
                                        G = n_genes
                                        n_markers = as.integer(0.3*n_genes) # used to be 0.1 
                                        n_ctsvg = as.integer(0.3*n_genes) # used to be 0.05
                                        message(paste0("Simulation with ", n_cell_types, " cell types | ", n_side^2, " spots | ",
                                                       n_genes, ",", n_markers, ",", n_ctsvg, " genes..."))
                                        
                                        # Step 1: Cell coordinates
                                        pp <- rpoispp(lambda = n_cells, win = owin(c(0, 1), c(0, 1)))
                                        ...
                                        # Step 3: Cell types
                                        coords$type <- sample(1:n_cell_types, size = nrow(coords), replace = TRUE, prob = .self$cell_type_proportion)
                                        ...
                                        for (k in 1:n_cell_types) {
                                          # --- marker 
                                          marker_idx <- sample(1:G, size = n_markers, replace = FALSE)
                                          ...
                                          svg_idx <- sample(1:G, size = n_ctsvg, replace = FALSE)
                                          mu[type_k & sd_k, svg_idx] <- 4
                                        ...
                                        # Spot composition
                                        comp_mat <- matrix(0, nrow = nrow(df_spot), ncol = n_cell_types)
                                        colnames(comp_mat) <- paste0("Type", 1:n_cell_types)
