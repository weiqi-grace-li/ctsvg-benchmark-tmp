...
# --- Step 1.1 & 2.1: load raw data and select one patient
core.spe <- readH5AD("./data/real_raw/kidney/visium_merge_core_upload.h5ad", reader = "R")
ref.sce <- readH5AD("./data/real_raw/kidney/RCC_upload_final_raw_counts.h5ad", reader = "R")

core.spe
table(core.spe$donor_id)
colData(ref.sce)
table(ref.sce$patient)

core.select = core.spe[,core.spe$donor_id == "PD47171"]
ref.select = ref.sce[,ref.sce$patient == "PD47171"]

spot_counts = assay(core.select, "X")
cell_counts = assay(ref.select, "X")

# --- step 1.2: Gene ID alignment (cell reference only)
# Note: unlike lung, the kidney spatial data (core.select) already stores counts
# with Ensembl IDs; only the single-cell reference uses gene symbols and needs
# symbol-to-Ensembl conversion.
...
# --- step 1.4: Spatial location for remaining spots
spot_coords = reducedDim(core.select, "spatial")
...
# - sanity check: verify cell barcodes in filtered count matrix match original reference metadata order
...
kidney_core = list()
kidney_core$dispersion = 0
kidney_core$seed = 0
kidney_core$cell_type_proportion = colSums(weight_broad)/sum(weight_broad)
names(kidney_core$cell_type_proportion) = colnames(weight_broad)
kidney_core$cell_counts = cell_counts
kidney_core$cell_metadata = cell_metadata
kidney_core$spot_counts = spot_counts[,remaining_spots]
kidney_core$spot_coords = spot_coords[remaining_spots,]
kidney_core$spot_composition = weight_broad
special_genes = list()
for (ct in names(kidney_core$cell_type_proportion)){
  special_genes[[ct]] = list(
    marker = c(""), ct_svg = c("")
  )
}
kidney_core$special_genes = special_genes
kidney_core$gene_names = rownames(spot_counts)
str(kidney_core)

# --- final packaged output: lightly-filtered packaged dataset
filter1 = kidney_core
save(filter1, file = "./data/real_packaged/kidney_core/filter1.RData")

# --- Step 4.3: create tr.cell object for spvc
plot(kidney_core$spot_coords[,"x"], kidney_core$spot_coords[,"y"], pch = ".")
boundary.idx <- identify(kidney_core$spot_coords, order = TRUE)
kidney_core_bounary = kidney_core$spot_coords[boundary.idx$ind[order(boundary.idx$order)], ]
points(kidney_core_bounary, type = "l", col = "blue")
Tr.cell <- TriMesh(kidney_core_bounary, n = 2)
Tr.cell = remove_colinear(Tr.cell)

# --- final packaged output: triangulation mesh for spvc
save(Tr.cell, file = "./data/real_packaged/kidney_core/Tr.cell.RData")
...
expressed_prop = 0.1
total_spots = ncol(kidney_core$spot_counts)
min_spot = ceiling(expressed_prop*total_spots)

spot_counts_filter = kidney_core$spot_counts>0
...
min_count = 100
spots_to_save = which(colSums(kidney_core$spot_counts) > min_count)
which(colSums(kidney_core$spot_counts) < min_count)

kidney_core_filtered = kidney_core

overlapping_gene = intersect(rownames(kidney_core$spot_counts)[genes_to_save], rownames(kidney_core$cell_counts))
kidney_core_filtered$cell_counts = kidney_core_filtered$cell_counts[overlapping_gene, ]
kidney_core_filtered$spot_counts = kidney_core_filtered$spot_counts[genes_to_save, ]
kidney_core_filtered$gene_names = rownames(kidney_core_filtered$spot_counts)
str(kidney_core_filtered)

# --- final packaged output: aggressively gene/spot-filtered packaged dataset
filter2 = kidney_core_filtered
save(filter2, file = "./data/real_packaged/kidney_core/filter2.RData")
